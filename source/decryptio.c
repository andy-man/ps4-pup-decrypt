...
ssize_t readbytes(const decrypt_state * state, size_t offset, size_t bytes, void * buffer, size_t buffersize) {
  ...
  if (offset != 0) {
      ...
      printf_notification("Seeked to position " SSIZET_FMT " in input file.\n", result);
  }
  ...
  if ((result == -1) || (bytesread != bytes)) {
      int errcode = errno;
      printf_notification("Read failed; Read " SSIZET_FMT " of " SSIZET_FMT "bytes - Result: %d (%s)\n", bytesread, bytes, errcode, strerror(errcode));
      return -1;
  }

//  printf_notification("Read " SSIZET_FMT " of " SSIZET_FMT " bytes to from input file.\n", bytesread, byte);
...
ssize_t writebytes(const decrypt_state * state, size_t offset, size_t bytes, void * buffer, size_t buffersize) {
  ...
  if (offset != 0) {
      ...
      printf_notification("Seeked to position " SSIZET_FMT " in output file.\n", result);
  }
  ...
  if ((result == -1) || (byteswritten != bytes)) {
      int errcode = errno;
      printf_notification("Write failed; Write " SSIZET_FMT " of " SSIZET_FMT "bytes - Result: %d (%s)\n", byteswritten,
												    bytes, errcode,
												    strerror(errcode));
      return -1;
  }

//  printf_notification("Write " SSIZET_FMT " of " SSIZET_FMT " bytes to output file.\n", byteswritten, bytes);
